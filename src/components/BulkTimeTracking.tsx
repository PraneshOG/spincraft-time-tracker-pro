...
const BulkTimeTracking = () => {
  ...
  const [employeeHours, setEmployeeHours] = useState<Record<string, EmployeeHoursData>>({});
  ...
  useEffect(() => {
    if (selectedDate !== lastSyncedDate) {
      ...
      const newHours: Record<string, EmployeeHoursData> = {};
  ...
  const updateEmployeeHours = (employeeId: string, hours: number) => {
    setEmployeeHours(prev => ({
      ...prev,
      [employeeId]: {
        ...(prev[employeeId] || { status: 'present' as const }),
        hours
      }
    }));
  };
  ...
  const handleCalculateSalary = async () => {
    try {
      ...
      workLogsForSalary?.forEach((log: any) => {
        ...
        employeeTotals[empId].total_hours += log.total_hours;
      });
                ...
                {employees.map(employee => {
                  ...
                  return (
                    <TableRow key={employee.id} className="hover:bg-accent/50">
                      ...
                      <TableCell>
                        <Select value={status} onValueChange={(value: 'present' | 'absent' | 'overtime' | 'holiday') => updateEmployeeStatus(employee.id, value)}>
